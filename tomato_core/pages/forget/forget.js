...
Page({
  ...
  confirm: function () {
    ...
    wx.request({
      url: WebService.HOST + WebService.FORGET_PASSWD_URL,
      data: {
        account_userphone: Base64.encode(userid),
        account_code: validCode,
        account_newpassword: Base64.encode(passwd)
      },
      success: function (data){
        if (data && data.data.code == 0){
          // utils.showToast('密码重置成功,请重新登录',5000);
          wx.showModal({
            title: '提示',
            content: '密码重置成功',
            showCancel: false,
          ...
          utils.showErrorModal('错误', '密码重置失败，请稍候再试!');
        }
      }
    })
  },
